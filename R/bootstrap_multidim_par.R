...
bootstrap_multidim_par <- function (x, y, B, n) {

  d <- ncol( x )
  b <- nrow( x )
  ans <- .C("bootstrap_multidim_par", as.double( x ), as.double( y ), result = as.double (numeric ( d )) , as.integer( b ), as.integer (B), as.integer( n ), as.integer( d ) )
  return(ans$result)
